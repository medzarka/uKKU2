...
class _page_measuerement_reviewers(Abstract_UI_Page):
    def __init__(self, request, link):
        super().__init__(page_title='MEASUREMENT :: Reviewers Administration', link=link,
                         request_obj=request)

    def CreateBlocks(self, _blocks_list=None):

        _admin_group_name = 'MEASUREMENT_HEAD'
        _reviewers_group_name = 'MEASUREMENT'
        ...
        if page_permission:
            ...
            for _user in _all_users:
                if _user.username == 'admin':
                    continue
                try:
                    MeasurementReviewerAffectations.objects.get(user=_user, semester=_actual_semester)
                except:
                    _tmp = MeasurementReviewerAffectations(user=_user, semester=_actual_semester, reviewer=None)
                    _tmp.save()
            ...
            if self.request_obj.method == 'POST':
                ...
                _tmp = MeasurementReviewerAffectations.objects.get(user=_user, semester=_actual_semester)
            ...
            _list = MeasurementReviewerAffectations.objects.filter(semester=_actual_semester)
            if len(_list) > 0:
                _ui_table_block = ui_basic_block(
                    block_title='Measurement Reviewers Affectation')
            ...
            for _aff in MeasurementReviewerAffectations.objects.filter(semester=_actual_semester).order_by('user'):
            ...
            _text1 = ui_text_element(
                text='The actual page is only for Measurement Committee Heads.',
